...
public class QihooCliClient extends CliClient {
	 ...
	 */
	@VisibleForTesting
	public QihooCliClient(Terminal terminal, String sessionId, Executor executor) {
		super(terminal, sessionId, executor);
	}
	 ...
	 * afterwards using {@link #close()}.
	 */
	public QihooCliClient(String sessionId, Executor executor) {
	...
	private void callCommand(SqlCommandCall cmdCall) {
		switch (cmdCall.command) {
			...
			default:
				...
				PrivateUtil.invoke(this, "callCommand", new Class[]{SqlCommandCall.class}, new Object[]{cmdCall});
	...
	private void callUseCatalog(SqlCommandCall cmdCall) {
		PrivateUtil.invoke(this, "callUseCatalog", new Class[]{SqlCommandCall.class}, new Object[]{cmdCall});
		getExecutor().setSessionProperty(getSessionId(), "execution.current-catalog", cmdCall.operands[0]);
	}

	private void callUseDatabase(SqlCommandCall cmdCall) {
		PrivateUtil.invoke(this, "callUseDatabase", new Class[]{SqlCommandCall.class}, new Object[]{cmdCall});
		getExecutor().setSessionProperty(getSessionId(), "execution.current-database", cmdCall.operands[0]);
	}

	private void callCreateFunction(SqlCommandCall cmdCall) {
		final String name = cmdCall.operands[0];
		final String functionClassName = cmdCall.operands[1].replace("\"", "").replace("'", "");
		final String language = cmdCall.operands[2];

		try {
			((QihooLocalExecutor) getExecutor()).addFunction(getSessionId(), name, functionClassName, language);
			printInfo("function " + name + " has been created.");
		} catch (Exception e) {
			printException("create function " + name, e);
		}
	}
	...
	private void callDropFunction(SqlCommandCall cmdCall) {
		final String name = cmdCall.operands[0];
		try {
			((QihooLocalExecutor) getExecutor()).removeFunction(getSessionId(), name);
			printInfo(CliStrings.MESSAGE_TABLE_REMOVED);
		} catch (Exception e) {
			printException("exec drop function "+ name, e);
		}
	}
